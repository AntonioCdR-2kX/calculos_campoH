...
% ********                       Descripción:                      ********
% Analisis del diseño 13 con 0º de desfase y 50 mm de Gap 
...
data=readtable('T13_RZ_0y50.csv');
%Renombramos las variables que exportamos de Ansys
H_RZ0y50 = data.Mag_H_kA_per_m__Freq__1MHz_Phase__0deg_;
x_RZ0y50 = data.Distance_mm_;

% Obtengo la simulación en el eje X y Y
data=readtable('T13_XY_0y50.csv');
%Renombramos las variables que exportamos de Ansys
H_XY0y50 = data.Mag_H_kA_per_m__Freq__1MHz_Phase__0deg_;
x_XY0y50 = data.Distance_mm_-180;

% Falta la simulación en 3D
data=readtable('T13_3D_0y50.csv');
%Renombramos las variables que exportamos de Ansys
H_3D0y50 = data.Mag_H_kA_per_meter__Freq__1MHz_Phase__0deg_;
x_3D0y50 = data.Distance_mm_-180;
...
% RZ
subplot(2,2,1);
title('Comparación RZ, XY, 3D y Matlab, con el número optimo de iteraciones en RZ: 8');
xlabel('x (mm)');
ylabel('H (A/mm)');
grid on;
hold on;
...
%
plot(x_RZ0y50,H_RZ0y50,'r-.','DisplayName','H_R_Z');
plot(x_XY0y50,H_XY0y50,'b-.','DisplayName','H_X_Y');
plot(x_3D0y50,H_3D0y50,'c-.','DisplayName','H_3_D');


% XY
subplot(2,2,2);
title('Comparación RZ, XY, 3D y Matlab, con el número optimo de iteraciones en XZ: 2');
xlabel('x (mm)');
ylabel('H (A/mm)');
grid on;
hold on;
...
% 3D
subplot(2,2,3);
title('Comparación RZ, XY, 3D y Matlab, con el número optimo de iteraciones en 3D: 6');
xlabel('x (mm)');
ylabel('H (A/mm)');
grid on;
hold on;
...
sgtitle('Modelo 13, Desfase: 0º, Gap: 50mm');


% Varias iteraciones
figure;
title('Comparación RZ, XY, 3D y Matlab con múltiple iteraciones', 'Modelo 13, Desfase: 0º, Gap: 50mm');
xlabel('x (mm)');
ylabel('H (A/mm)');
grid on;
hold on;
