...
% ********                       Descripción:                      ********
% Analisis del diseño 10 con 0º de desfase y 100 mm de Gap 
...
data=readtable('T10_RZ_0y100.csv');
...
data=readtable('T10_XY_0y100.csv');
%Renombramos las variables que exportamos de Ansys
H_XY0y100 = data.Mag_H_kA_per_m__Freq__1MHz_Phase__0deg_;
x_XY0y100 = data.Distance_mm_-150;
...
% XY
subplot(2,2,2);
title('Comparación RZ, XY, 3D y Matlab, con el número optimo de iteraciones en XZ: 1');
xlabel('x (mm)');
ylabel('H (A/mm)');
grid on;
hold on;
...
sgtitle('Modelo 10, Desfase: 0º, Gap: 100mm');


% Varias iteraciones
figure;
title('Comparación RZ, XY, 3D y Matlab con múltiple iteraciones', 'Modelo 10, Desfase: 0º, Gap: 100mm');
xlabel('x (mm)');
ylabel('H (A/mm)');
grid on;
hold on;
